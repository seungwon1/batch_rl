...
def show_process(FLAGS, episode_count ,rew_epi, global_avg_reward, best_reward, loss_epi, eps, learning_rate, step_count, step_start, 
    ...
    if os.path.exists('results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)) == False:
        os.makedirs('results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed))
        os.makedirs('tmp/'+str(FLAGS.arch)+'/'+str(FLAGS.seed))
    ...
    if episode_count % FLAGS.print_every == 0:
        ...
        plt.legend()
        plt.savefig('./results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)+'/it_frame_reward')
        plt.clf()
        
        plt.plot(step_his, mean_reward, label = 'avg reward')
        #plt.ylim(-30, 25)
        plt.legend()
        plt.savefig('./results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)+'/it_frame_rew_per_step')
        plt.clf()
                            
        plt.plot(loss_his, label = 'loss')
        plt.title('Iteration frame: '+ str(step_count))
        plt.legend()
        plt.savefig('./results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)+'/it_frame_loss')
        plt.clf()
               
        if episode_count % 200 == 0 or step_count == FLAGS.max_frames:
            if step_count > 500000: #1000000
                np.save('./results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)+'/replay_memory', exp_memory.memory_frame) 
                np.save('./results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)+'/replay_memory2', exp_memory.memory_a_r)   
                np.save('./results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)+'/loss_his', loss_his)
                np.save('./results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)+'/mean_reward', mean_reward) 
                np.save('./results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)+'/step_his', step_his) 
                np.save('./results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)+'/step_start', step_start)
                np.save('./results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)+'/reward_his', reward_his)
...
def reload_session(sess, saver, exp_memory, FLAGS):
    ...
    exp_memory.memory_frame = np.load('./results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)+'/replay_memory.npy')       
    exp_memory.memory_a_r = np.load('./results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)+'/replay_memory2.npy')
    loss_his = np.load('./results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)+'/loss_his.npy').tolist()
    reward_his = np.load('./results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)+'/reward_his.npy').tolist()
    mean_reward = np.load('./results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)+'/mean_reward.npy').tolist() # mean_reward
    step_his = np.load('./results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)+'/step_his.npy').tolist()
    step_start = np.load('./results/'+str(FLAGS.arch)+'/'+str(FLAGS.seed)+'/step_start.npy').tolist()

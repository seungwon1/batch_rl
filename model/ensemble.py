...
class ENS_DQN(DQN):
    ...
    def model(self, network):
        ...
        with tf.variable_scope(network):
            ...
            est_q = tf.reduce_sum(out * action_mask, axis = 1)# of shape (N,num_heads) which is ensemble model of Q(s,a)
            
        if network == 'online':
            return state, action, est_q, greedy_idx, None 
        
        elif network == 'target':
            update_est_q = tf.reshape(batch_rew, [-1, 1]) + self.gamma*tf.reshape((1-batch_done_idx),[-1, 1])* est_q # of shape (N,num_heads)
            return batch_ns, batch_rew, batch_done, update_est_q
    
    def loss_fn(self, target_q, online_q): # use huber loss
        err = tf.stop_gradient(target_q) - online_q # of shape (N, num_heads)
        loss = tf.reduce_mean(tf.reduce_sum(self.huber_loss(err), axis = 1))
        return loss
